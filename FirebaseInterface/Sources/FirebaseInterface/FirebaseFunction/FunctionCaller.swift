...
internal struct FunctionCaller {
    ...
    private func createParameters(
        of functionCall: some FunctionCallable
    ) throws -> [String: Any] {
    ...
    ) async throws -> HTTPSCallableResult where FunctionCall: FunctionCallable {
    ...
    public func call<FunctionCall>(
        _ functionCall: FunctionCall
    ) async throws -> FunctionCall.ReturnType where FunctionCall: FunctionCallable {
    ...
    public func call<FunctionCall>(
        _ functionCall: FunctionCall
    ) async throws where FunctionCall: FunctionCallable, FunctionCall.ReturnType == VoidReturnType {
    ...
    public func privateKey(_ privateKey: String) -> FunctionCaller {
        var functionCaller = self
        functionCaller.privateKey = privateKey
        return functionCaller
    }
    
    /// Sets the database type of the returned function called to `testing`.
    public var forTesting: FunctionCaller {
        var functionCaller = self
        functionCaller.databaseType = .testing
        return functionCaller
    }
}
